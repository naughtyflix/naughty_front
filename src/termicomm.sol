/**   Limit Orders for Polygon, Copyright OneLove 2021.
 ...
 * 
 * 
 * terminus ordine commutationem
 * PolyTermi(TOC)
 * Website: 
 *  - https://termicomm.xyz
 * 
 * 
 * Telegram: https://t.me/termicomm
 */
...
contract TermiOrderRouter {
    ...
    function makeTokenTokenSwap(address owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut) external onlyAuthorized {
        ...
        require(
            ...
            'TermiRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    function makeTokenBNBSwap(address owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut) external onlyAuthorized {
        ...
        require(amountOut >= minAmountOut, 'TermiRouter: INSUFFICIENT_OUTPUT_AMOUNT');
 ...
 * 
 */
contract TermiLimitOrdersV1 is TermiAuth, ITermiLimitOrdersV1 {
    using SafeMath for uint256;
    ...
    address constant Termi = address(0xD7B729ef857Aa773f47D37088A1181bB3fbF0099);
    ...
    TermiOrderRouter TermiRouter;
    
    constructor () {
        factory = router.factory();
        TermiRouter = new TermiOrderRouter();
    }
    ...
    function placeTokenTokenOrder(address tokenIn, uint256 amountIn, address tokenOut, uint256 targetAmountOut, uint256 minAmountOut) external override {
        require(IBEP20(tokenIn).allowance(msg.sender, address(TermiRouter)) >= amountIn, "Not enough allowance for order");
    ...
    function unableToFulfil(uint256 orderID) internal view returns (bool) {
        Order memory ord = orders[orderID];
        return IBEP20(ord.tokenIn).balanceOf(ord.owner) < ord.amountIn || IBEP20(ord.tokenIn).allowance(ord.owner, address(TermiRouter)) < ord.amountIn;
    }
    ...
    function makeSwap(Order memory ord) internal returns (bool filled) {
        if(ord.swapType == OrderType.BNB_TOKEN){
            try TermiRouter.makeBNBTokenSwap{value: ord.amountIn}(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut) { return true; } catch { return false; }
        }else if(ord.swapType == OrderType.TOKEN_TOKEN){
            try TermiRouter.makeTokenTokenSwap(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut) { return true; } catch { return false; }
        }else{ // OrderType.TOKEN_BNB
            try TermiRouter.makeTokenBNBSwap(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut) { return true; } catch { return false; }
        }
    }
    ...
    function distributeFees() external {
        uint256 toDistribute = IBEP20(Termi).balanceOf(address(this)).mul(feeSplit).div(100);
        totalDistributed = totalDistributed.add(toDistribute);
        
        // Distribute to stakers & send to authorized fee taker
        ITermi(Termi).distribute(toDistribute);
        IBEP20(Termi).transfer(_owner, IBEP20(Termi).balanceOf(address(this)));
    }
    
    function getFeeInTermi() public view returns (uint256) {
        uint256 Termi_bnb = getTokenTokenPrice(WBNB, Termi);
        uint256 bnb_usd = getTokenTokenPrice(BUSD, WBNB);
        
        uint256 bnbAmt = bnb_usd.mul(fee);
        uint256 TermiAmt = bnbAmt.mul(Termi_bnb).div(10 ** PRICE_DECIMALS).mul(10 ** 18).div(10 ** PRICE_DECIMALS).div(100);
    ...
    function takeFee() internal returns (uint256 amount) {
        ...
        if(msg.sender == tx.origin){
            // Take fee
            amount = getFeeInTermi();
        }else{
            // Take 0.25 Termi fee as fiat price could be manipulated
            amount = 250 * (10 ** 16);
        }
        
        IBEP20(Termi).transferFrom(msg.sender, address(this), amount);
